...
import { DropDownSearch, useAuth } from '../../index.ts';
import { fetchNavbarSearch, type Anime } from '../../index.ts';
...
export const Navbar = () => {
  ...
  const fetchSearchResults = async (query: string) => {
    ...
    try {
      const fetchedData = await fetchNavbarSearch(query); // Fetch first 5 results for the dropdown
      const formattedResults = fetchedData.map((anime: Partial<Anime>) => ({
        id: anime.id || '',
