  ...
  return streams.filter(stream => {
    ...
    if (langFilter !== 'all') {
      ...
      if (filterConfig && filterConfig.langKey) {
        // Check stream's langKey property
        if (stream.langKey && stream.langKey !== filterConfig.langKey) {
          return false
        }
        // Also check stream's language.code property
        if (stream.language && stream.language.code && !stream.language.code.includes(filterConfig.code)) {
          return false
        }
      }
    }
...
export default function PluginAnimePage() {
  ...
  const handleEpisodeClick = async (episode) => {
    ...
    // Don't load streams immediately - wait for user to click on a specific hoster
    // This implements lazy loading as requested
  }

  // Fetch streams for a specific hoster
                    ...
                    {isExpanded && (
                      <div className="border-t border-[#2a2a2d] p-3">
                          ...
                          <div className="grid gap-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {(episode.hosters || episode.providers || []).map((hoster, hosterIdx) => {
                                const hosterName = getDisplayName(hoster)
                                const isLoadingThis = loadingHoster === hosterName
                                return (
                                  <Button
                                    key={hosterIdx}
                                    size="1"
                                    variant="soft"
                                    color="gray"
                                    onClick={() => handleHosterClick(episode, hosterName)}
                        ...
                        {!isLoadingThisEpisode && streams && streams.length > 0 && (() => {
                          // Get available languages and hosters for filter dropdowns
                          const availableStreamLangs = getAvailableStreamLanguages(streams)
                          const availableHosters = getUniqueHosters(streams)
                          const hasFilters = availableStreamLangs.length > 1 || availableHosters.length > 1
                          
                          // Apply filters
                          const filteredStreams = filterStreams(streams, streamLanguageFilter, streamHosterFilter)
